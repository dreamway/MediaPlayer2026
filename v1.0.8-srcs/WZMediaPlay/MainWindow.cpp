    ...
    : QMainWindow(parent)
{
    ...
    GlobalDef::getInstance()->VIDEO_FILE_TYPE << ".mp4"<< ".flv"<< ".f4v"<< ".webm"<< ".m4v"<< ".mov"<< ".3gp"<< ".3g2"<< ".rm"<< ".rmvb"<< ".wmv"<< ".avi"
                                              << ".asf"<< ".mpg"<< ".mpeg"<< ".mpe"<< ".ts"<< ".div"<< ".dv"<< ".divx"<< ".vob"<< ".mkv"<<".wzmp4"<<".wzavi"
                                              <<".wzmkv"<<".wzmov"<<".wzmpg";
    ...
    //  cpu mac 验证
    {
        ...
        if (readFile.open(QIODevice::ReadOnly)) {
            ...
            // 未注册情况下，仍可试用一段时间
            QDateTime startUsingTime = QDateTime(QDate(2026, 03, 01), QTime(0, 0, 0, 0));
            QDateTime currentTime = QDateTime::currentDateTime();
            qint64 elapsedDay = startUsingTime.daysTo(currentTime);
            const int MAX_EVALUATE_DAY = 90;
            const int WARNING_EXPIRED_DAY = 30;
    ...
    // 创建Movie（可认为是Data, 与FFmpegView不要绑定在一块)
    movie_ = new Movie;
    connect(
        movie_,
        SIGNAL(playStateChanged(PlayState)),
        this,
        SLOT(onPlayStateChanged(PlayState))); // movie->startMutex为单独的渲染线程，与GLRenderer不能用DirectConnection,否则不同线程停Timer会失效。

    connect(movie_, &Movie::seekingFinished, this, &MainWindow::onSeekingFinished);
    logger->info("after setup movie...");

    ui.playWidget->SetRelativeMovie(movie_);

    movie_->SetVolume((float) ui.horizontalSlider_volume->value() / (float) ui.horizontalSlider_volume->maximum());
...
bool MainWindow::setupLogger()
{
    ...
    logger->info("logMode:{}, logLevel:{}", GlobalDef::getInstance()->LOG_MODE, GlobalDef::getInstance()->LOG_LEVEL);
...
void MainWindow::initUI()
{
    ...
    connect(ui.playWidget, &FFmpegView::updatePlayProcess, this, &MainWindow::onUpdatePlayProcess);
...
bool MainWindow::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
{
    ...
    case WM_WTSSESSION_CHANGE: //判断登录、注销、锁屏等
    {
        ...
        case WTS_SESSION_LOCK:
            ...
            playerIsPlayingBeforeScreenLockOrPowerSleep = movie_->IsPlaying();
            if (playerIsPlayingBeforeScreenLockOrPowerSleep) {
                bool shouldPause = true;
                movie_->PlayPause(shouldPause);
...
void MainWindow::openFile()
{
    QString filepath = QFileDialog::getOpenFileName(
        ...
        "video(*.mp4 *.flv *.f4v *.webm *.m4v *.mov *.3gp *.3g2 *.rm *.rmvb *.wmv *.avi *.asf *.mpg *.mpeg *.mpe *.ts *.div *.dv *.divx *.vob *.mkv *.wzmp4 *.wzavi *.wzmkv *.wzmov *.wzmpg)"
    );
    if (!filepath.isEmpty()) {
...
int MainWindow::openPath(QString path, bool addPlayList)
{
    ...
    // 优化：使用更智能的等待机制，避免固定 sleep 导致的延迟
    // 由于 Movie::stop() 已经提前设置 playState = STOP，IsStopped() 应该能立即返回 true
    ...
    while (false == movie_->IsStopped() && waitAttempts < MAX_WAIT_ATTEMPTS) {
        //用状态可能切换不对，改用内部的quit_变量，若在打开时，上一个视频还没有退出，则应该需要调用stop将上一个视频退出后再开新视频
        if (waitAttempts == 0) {
            logger->info(" movie_->IsStopped is false, or Movie IsOpened, when switching, need stop. ");
            movie_->Stop();
            switchStop = true;
        }
        ...
        if (waitAttempts % 10 == 0) {
            logger->debug("Waiting for movie to stop, attempts: {}/{}, elapsed: {}ms", 
                         waitAttempts, MAX_WAIT_ATTEMPTS, elapsed);
        }
    }
    
    if (waitAttempts >= MAX_WAIT_ATTEMPTS) {
        logger->warn("Stop wait reached max attempts ({}), forcing continue (video thread may need more time to exit)", 
                    MAX_WAIT_ATTEMPTS);
    }

    if (switchStop) {
        // 由于 Movie::stop() 已经提前设置 playState = STOP，这里通常不需要等待
        // 但为了确保状态完全同步，再检查一次（使用更短的等待时间）
        if (false == movie_->IsStopped()) {
        ...
        SPDLOG_LOGGER_INFO(logger, "movie_->IsStopped is True, no need to wait.");
    }
    ...
    if (mLastMovieFn == path) {
        ...
        if (ui.playWidget) {
            logger->info("Switching to new video file, stopping previous subtitle. LastMovie: {}, NewMovie: {}",
                        mLastMovieFn.toUtf8().constData(), path.toUtf8().constData());
            // 停止并清除字幕
    ...
    currentElapsedInSeconds_ = 0;
    int openCode = movie_->Open(path);
    if (openCode < 0) {
        logger->error("movie_->Open failed, openCode:{}", openCode);
        return openCode;
    }
    mCurMovieFilename = path;

    movie_->SetSeekFlag(false);
    ...
    //  set TotalTime
    int mvTotalTimeSecond = movie_->GetDurationInMs() / 1000;
    ...
    float frameRate = movie_->GetVideoFrameRate();
    if (addPlayList) {
        QStringList addList;
        addList.append(path);
    ...
    // 等待视频预加载完成，避免高分辨率高码率视频刚打开时卡顿
    // 预加载会先解码一定数量的帧到队列中，然后再开始渲染
    logger->info("Waiting for video preload before starting rendering...");
    movie_->waitForVideoPreload(10000);  // 最多等待10秒
    logger->info("Video preload completed, starting rendering");
    ...
    for (int i = 0; i < GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[GlobalDef::getInstance()->PLAY_LIST_DATA.playlist_current_index].video_list.size();
         ++i) {
        if (GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[GlobalDef::getInstance()->PLAY_LIST_DATA.playlist_current_index].video_list[i].video_path
            == path) {
            if (ui.tabWidget_playList->currentIndex() == 0) {
                ui.listWidget_playlist->clearSelection();
                ui.listWidget_playlist->setCurrentRow(i);
            } else {
                ((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->clearSelection();
                ((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->setCurrentRow(i);
            }
            break;
        }
    }
...
void MainWindow::updatePlayList(QStringList addList, int playListIndex)
{
    for (int i = 0; i < addList.size(); ++i) {
        bool b_haveVideo = false;
        for (int j = 0; j < GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[playListIndex].video_list.size(); ++j) {
            if (GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[playListIndex].video_list[j].video_path == addList[i]) {
                b_haveVideo = true;
                break;
            }
        }
        if (!b_haveVideo) {
            QString fileType = addList[i].right((addList[i].size() - addList[i].lastIndexOf(".")));
            if (!GlobalDef::getInstance()->VIDEO_FILE_TYPE.contains(fileType, Qt::CaseInsensitive))
                continue;
            VideoInfo item;
            item.video_path = addList[i];
            GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[playListIndex].video_list.push_back(item);

            QString mvPath = addList[i];
            mvPath = mvPath.replace("\\", "/");
            QStringList mvPathArr = mvPath.split("/");
            if (GlobalDef::getInstance()->PLAY_LIST_DATA.playlist_current_index == 0) {
                ui.listWidget_playlist->addItem(
                    QString::number(GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[playListIndex].video_list.size()) + "."
                    + mvPathArr[mvPathArr.size() - 1]);
            } else {
...
{
    logger->info("playWidget isPlaying:{}, isPause:{}", movie_->IsPlaying(), movie_->IsPaused());

    if (movie_->IsPlaying()) {
        ui.playWidget->PlayPause(true);
        ui.pushButton_playPause->setChecked(false);
    } else if (movie_->IsPaused()) {
        ui.playWidget->PlayPause(false);
        ui.pushButton_playPause->setChecked(true);
    } else {
        if (ui.tabWidget_playList->currentIndex() == 0) {
...
{
    ui.playWidget->stopCamera();
    ui.playWidget->closeCamera();
}
...
void MainWindow::dropEvent(QDropEvent *event)
{
    ...
    if (movie_->IsPaused() || movie_->IsPlaying()) {
...
void MainWindow::on_actionOpenCamera_toggled(bool cam_checked)
{
    ...
    if (cam_checked) {
        QList<QCameraDevice> cameras = ui.playWidget->getCamerasInfo();
        // qDebug() << __FUNCTION__ << "..............ABC, camera.size:"<<cameras.size();
        StereoFormat stereoFormat = StereoFormat::STEREO_FORMAT_NORMAL_2D;
        StereoInputFormat stereoInputFormat = StereoInputFormat::STEREO_INPUT_FORMAT_LR;
        StereoOutputFormat stereoOutputFormat = StereoOutputFormat::STEREO_OUTPUT_FORMAT_VERTICAL;
        if (cameras.size() > 0) {
            ui.playWidget->openCamera(cameras[0]);
            ui.playWidget->startCamera();

            ui.playWidget->StartRendering(stereoFormat, stereoInputFormat, stereoOutputFormat, 30);
        }
    } else {
        ui.playWidget->stopCamera();
        ui.playWidget->closeCamera();
...
void MainWindow::reply_action_3D_hotKey()
{
    ...
    //ui.action_3D->setChecked(!ui.action_3D->isChecked());
    ui.action_3D->setChecked(true);
}
...
{
    logger->debug("elapsedInSeconds:{}", elapsedInSeconds);
    if (movie_->IsSeeking()) {
        logger->debug("Seeking, Not update play progress");
        return;
    }
...
{
    logger->info("onPlaySliderCustomClicked， seekTarget:{}", seekTarget);
    if (false == movie_->IsOpened()) {
        logger->warn("movie not opened, return");
        return;
    }
    if (ui.playWidget == nullptr) {
        logger->warn("playWidget is null, onPlaySliderCustomClicked with target:{} will failed.", seekTarget);
        return;
    }

    if (movie_->IsSeeking()) {
        logger->warn(" already seeking, {},   skip!!", seekTarget);
        return;
    }

    logger->info("currentElapsedInSeconds_:{},  seek_target:{}", currentElapsedInSeconds_, seekTarget);

    ui.playWidget->SetSeeking(__FUNCTION__, true);
    bool seekOk = movie_->SetSeek(seekTarget);
}

void MainWindow::onPlayProcessValueChanged(int value)
{
    logger->debug("OnPlayProgressValueChanged, currentElapsedInSeconds_ is:{}, new value:{}", currentElapsedInSeconds_, value);

    return;
}

// 因为Seeking在Movie的另一个线程，且需要花费一定时间，因此，直接在上述Seeking中的返回并不能保证真正Seeking完成。故建一个signal-slot进行连接
void MainWindow::onSeekingFinished(int64_t seek_target)
{
    logger->info("SeekFinished, value:{}", seek_target);
    //sleepMsec(300);

    ui.playWidget->SetSeeking(__FUNCTION__, false);

    int posInMs = seek_target / 1000000;
    ui.playWidget->UpdateSubtitlePosition(posInMs / 1000);
    currentElapsedInSeconds_ = posInMs;
...
{
    logger->info("playSliderMousePressEvent, currentValue:{}", ui.horizontalSlider_playProgress->value());
}

void MainWindow::onPlaySliderMouseMoveEvent(int value)
{
    logger->info(" playSliderMouseMoveEvent, value: {}", value);
}

void MainWindow::onPlaySliderMouseReleaseEvent()
{
    logger->info(" PlaySliderMouseReleaseEvent, release value:{}, ", ui.horizontalSlider_playProgress->value());

    return;
}

void MainWindow::onPlayStateChanged(PlayState state)
{
    logger->info("MainWindow.onPlayStateChanged, state:{}", int(state));

    if (state == PlayState::STOP) {
        //清除上一次播放视频关联的Subtitle文件（但需注意的是，保留上一次播放文件 及 上一次字幕，若检测到下一次播放的文件与上一次相同，则继续载入上一次字幕)
        mLastMovieFn = mCurMovieFilename;
        if (GlobalDef::getInstance()->USER_SELECTED_SUBTITLE_FILENAME.isEmpty() == false) {
            mLastSubtitleFn = GlobalDef::getInstance()->USER_SELECTED_SUBTITLE_FILENAME;
            //将上一次的字幕清除，以防止下一个视频采用上一个视频的字幕
        ...
        bool bIsPlayFinishedStop = false;
        int movieTotalMs = movie_->GetDurationInMs() / 1000;
        
        // 方法1：传统的时间差判断
        bool timeBasedFinished = (movieTotalMs - currentElapsedInSeconds_ < 3 && movieTotalMs > 3);
        
        // 方法2：使用新的综合检测方法（检查队列、线程状态等）
        bool comprehensiveFinished = movie_->isPlaybackFinished(currentElapsedInSeconds_);
        
        // 只要其中一种方法认为播放完成，就认为播放完成
        if (timeBasedFinished || comprehensiveFinished) {
            logger->info(
                "onPlayStateChanged is Stop, and the Stop is PlayFinishedStop. "
                "movieTotalMs:{}, currentElapsed:{}, timeBased:{}, comprehensive:{}", 
                movieTotalMs, currentElapsedInSeconds_, timeBasedFinished, comprehensiveFinished);
            // 对于正常完播，更新其状态
            bIsPlayFinishedStop = true;
        }

        if (false == bIsPlayFinishedStop) {
            // 并且对于非正常完播的视频，不适用于PlayListLoop的切换
            logger->warn("onPlayStateChanged is Stop, BUT this is not a NORMAL PlayFinishedStop, "
                        "Skip playlist switching. movieTotalMs:{}, currentElapsed:{}, "
                        "timeBased:{}, comprehensive:{}",
                        movieTotalMs, currentElapsedInSeconds_, timeBasedFinished, comprehensiveFinished);
            return;
        }
        ...
        case PlayLoop::PLAYLOOP_SINGLE_PLAY:
        default:
            logger->info("PLAYLOOP_SINGLE_PLAY, stopped.");
            // 当播放完成或停止时，状态更新Timer停止。 其余循环状态的情况，不停止playWidget, 这样会继续播放黑画面，修复#2，避免视频切换过程中会有LOGO闪现的情况
            ui.playWidget->StopRendering();
            break;
        case PlayLoop::PLAYLOOP_SINGLE_CYCLE:
            logger->info("PLAYLOOP_SINGLE_CYCLE, loop play this video:{} ", ui.listWidget_playlist->currentRow());
            QTimer::singleShot(1000, [&] {
                if (movie_->IsStopped()) {
                    logger->info("2222222222222222222222222222  is stop true");
                    on_pushButton_playPause_clicked();
                } else {
                    logger->info("33333333333333333333333333  is stop false");
                }
            });
            break;
        case PlayLoop::PLAYLOOP_SEQUENTIAL_PLAY:
            logger->info(
                "PLAYLOOP_SEQUENTIAL_PLAY, currentIndex:{} , videoListSize:{}, count:{}",
                ui.listWidget_playlist->currentRow(),
                GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list.size(),
                ui.listWidget_playlist->count());
            if (ui.tabWidget_playList->currentIndex() == 0) {
                int index = ui.listWidget_playlist->row(ui.listWidget_playlist->currentItem()) + 1;
                if (index < GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list.size()) {
                    if (index >= 0) {
                        QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list[index].video_path;
                        logger->info(" SequentialPlay,1.1 index:{}, current path:{}", index, mvPath.toUtf8().constData());
                        //ui.listWidget_playlist->setCurrentRow(index);
                        openPath(mvPath, false);
                    } else {
                        ui.pushButton_playPause->setChecked(false);
                        logger->info("1.1 current index is not valid:{}", index);
                    }
                } else {
                    logger->info(" 1.2 index:{} > size:{}, stop rendering", index, GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list.size());
                    ui.playWidget->StopRendering();
                }
            } else {
                int index = ((PlayListPage *) ui.tabWidget_playList->currentWidget())
                                ...
                                ->row(((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->currentItem())
                            + 1;
                if (index < GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list.size()) {
                    if (index >= 0) {
                        QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list[index].video_path;
                        logger->info("other tab,2.1 index:{} current path:{}", index, mvPath.toUtf8().constData());
                        ((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->setCurrentRow(index);
                        openPath(mvPath, false);
                    } else {
                        ui.pushButton_playPause->setChecked(false);
                        logger->info("2.1 current index is not valid:{}", index);
                    }
                } else {
                    logger->info(
                        " 2.2 index:{} < size {}, stop rendering",
                        index,
                        GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list.size());
                    ui.playWidget->StopRendering();
                }
            }
            break;
        case PlayLoop::PLAYLOOP_RANDOMLY_PLAY:
            logger->info("PLAYLOOP_RANDOMLY_PLAY, currentIndex:{} ", ui.listWidget_playlist->currentRow());
            if (ui.tabWidget_playList->currentIndex() == 0) {
                int index = QRandomGenerator::global()->bounded(GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list.size());
                if (index >= 0) {
                    QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list[index].video_path;
                    logger->info("1.1  current path:{}, random:{}", mvPath.toUtf8().constData(), index);
                    //ui.listWidget_playlist->setCurrentRow(index);
                    openPath(mvPath, false);
                } else {
                    ui.pushButton_playPause->setChecked(false);
                    logger->debug("1.2  current random index:{} [should not enter here]", index);
                }
            } else {
                ...
                if (index >= 0) {
                    QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list[index].video_path;
                    logger->debug("2.1 current path:{}", mvPath.toUtf8().constData());
                    ((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->setCurrentRow(index);
                    openPath(mvPath, false);
                } else {
                    ui.pushButton_playPause->setChecked(false);
                    logger->info("2.2 index:{} <size, stop rendering", index);
                }
            }
            break;
        case PlayLoop::PLAYLOOP_LIST_LOOP:
            logger->info("PLAYLOOP_LIST_LOOP, currentIndex:{} ", ui.listWidget_playlist->currentRow());
            if (ui.tabWidget_playList->currentIndex() == 0) {
                int index = ui.listWidget_playlist->row(ui.listWidget_playlist->currentItem()) + 1;
                index = index >= GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list.size() ? 0 : index;
                if (index >= 0) {
                    QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[0].video_list[index].video_path;
                    logger->info("PLAYLOOP_LIST_LOOP, 1.1 current path:{}, index:{}", mvPath.toUtf8().constData(), index);
                    ui.listWidget_playlist->setCurrentRow(index);
                    openPath(mvPath, false);
                } else {
                    ui.pushButton_playPause->setChecked(false);
                    logger->info("1.2 index:{} invalid, stop rendering", index);
                }
            } else {
                int index = ((PlayListPage *) ui.tabWidget_playList->currentWidget())
                                ->getListWidget()
                                ->row(((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->currentItem())
                            + 1;
                index = index >= GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list.size() ? 0 : index;
                if (index >= 0) {
                    QString mvPath = GlobalDef::getInstance()->PLAY_LIST_DATA.play_list[ui.tabWidget_playList->currentIndex()].video_list[index].video_path;
                    logger->info("PlayLoop(List), 2.1 current path:{}, index:{}", mvPath.toUtf8().constData(), index);
                    ((PlayListPage *) ui.tabWidget_playList->currentWidget())->getListWidget()->setCurrentRow(index);
                    openPath(mvPath, false);
                } else {
                    ui.pushButton_playPause->setChecked(false);
                    logger->info("2.2 index:{} invalid, stop rendering", index);
                }
            }
            break;
        }
...
{
    if (ui.playWidget && movie_->IsOpened()) {
...
#ifdef INSPECT
void MainWindow::onInspectTimerTimeout()
{
    if (movie_) {
        logger->debug(
            "  current movie_ state:{}, GlobalInputFormat:{}, GlobalOutputFormat:{}",
            int(movie_->GetPlayState()),
            int(GlobalDef::getInstance()->DefaultStereoInputFormat),
            int(GlobalDef::getInstance()->DefaultStereoOutputFormat));
    }
    if (ui.playWidget) {
        ui.playWidget->DebugPrintStatus();
    }
}
#endif
