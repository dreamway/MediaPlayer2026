...
TcpSocket::TcpSocket(int socketdes, QTcpSocket *parent)
    : QTcpSocket(parent)
{
    this->setSocketDescriptor(socketdes);
    ...
    if (bytes.size() >= 9) {
        if (bytes[0] != (char) 0xA3 || bytes[bytes.size() - 1] != (char) 0x3A) {
            ...
            if ((len + 9) == bytes.size()) {
                ...
                //qDebug() << "verSum :" << verSum << "  verSend:" << verSend;
                ...
                if (verSum == verSend) {
                    ...
                    //memcpy(&temp, &sumA, 2);
                    temp.setNum(sumA);
                    qDebug() << "temp size:" << temp.size() << "  returnBytes size:" << returnBytes.size() << "  sumA:"<<sumA
...
void TcpSocket::SocketErr(QAbstractSocket::SocketError socketErr){
    qDebug() << __FUNCTION__;
}