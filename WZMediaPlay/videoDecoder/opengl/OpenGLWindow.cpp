    ...
    , m_passEventsToParent(m_platformName != "xcb" && m_platformName != "android")
{
    ...
    // TODO: 如果需要，可以连接视频窗口可见性信号
    // connect(&videoDockVisibleSignal, SIGNAL(...), this, SLOT(videoVisible(bool)));
}
OpenGLWindow::~OpenGLWindow()
{
    makeCurrent();
...
bool OpenGLWindow::event(QEvent *e)
{
    switch (e->type())
    {
        ...
        case QEvent::Wheel:
            if (m_passEventsToParent)
                // TODO: 如果需要，可以转发事件到视频窗口
                // return QCoreApplication::sendEvent(const_cast<QWidget *>(getVideoDock()), e);
                return false;
        default:
            break;
    }
