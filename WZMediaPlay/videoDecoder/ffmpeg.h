...
// No-op deleter for cases where ownership is managed elsewhere
struct AVFormatContextNoOpDeleter {
    void operator()(AVFormatContext *ptr) { /* Do nothing, ownership managed elsewhere */ }
};
using AVFormatContextPtr =
        std::unique_ptr<AVFormatContext, AVFormatContextDeleter>;
using AVFormatContextPtrNoOp =
        std::unique_ptr<AVFormatContext, AVFormatContextNoOpDeleter>;
